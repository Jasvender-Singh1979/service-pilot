...
function EditServiceCallContent() {
  const { user } = useAuth();
  ...
  // Load form data and service call - only once on mount
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!callResponse.ok) throw new Error('Failed to load service call');
        ...
        // Populate form with existing data - ONLY on initial load
        setForm({
          customer_name: callData.customer_name || '',
          customer_address: callData.customer_address || '',
          phone_country_code: callData.phone_country_code || '+91',
          customer_phone: callData.customer_phone || '',
          whatsapp_country_code: callData.whatsapp_country_code || '+91',
          customer_whatsapp: callData.customer_whatsapp || '',
          whatsapp_same_as_phone: callData.whatsapp_same_as_phone || false,
          category_id: callData.category_id || '',
          problem_reported: callData.problem_reported || '',
          priority_level: callData.priority_level || '',
          purchase_source: callData.purchase_source || '',
          seller_name_if_other: callData.seller_name_if_other || '',
          warranty_status: callData.warranty_status || '',
          purchase_date: callData.purchase_date ? callData.purchase_date.split('T')[0] : '',
          charge_type: callData.charge_type || '',
          custom_amount: callData.custom_amount ? String(callData.custom_amount) : '',
        ...
        if (isMounted) {
          setError('Failed to load service call data');
          setLoading(false);
        }
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to update service call');
      }

      // Success - return to detail view with source context preserved
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to update service call'
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 pb-24">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Edit Service Call</h1>
          <p className="text-sm text-slate-500 mt-1">
            Update the service call details
          </p>
        </div>
      </div>
...
export default function EditServiceCallPage() {
  return (
    <ProtectedRoute requiredRole="manager">
      <Suspense fallback={<div className="flex items-center justify-center min-h-screen">Loading...</div>}>
        <EditServiceCallContent />
      </Suspense>
    </ProtectedRoute>
  );
}
