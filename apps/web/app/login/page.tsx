...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[LoginPage] Sign-in successful, enriched user:", userData);
      
      // Use role from enriched user data
      const userRole = userData?.role || userData?.data?.user?.role;
      
      if (!userRole) {
        console.error("[LoginPage] CRITICAL: User role is undefined after enrichment", {
          userData,
          rawRole: userData?.role,
          dataRole: userData?.data?.user?.role,
        });
        setError('Failed to load user profile. Please try again.');
        return;
      }

      console.log("[LoginPage] User role:", userRole, "- Redirecting...");
      ...
      if (userRole === 'super_admin') {
        ...
        console.error("[LoginPage] Unknown role:", userRole);
        setError(`User role not recognized: ${userRole}`);
      }
    } catch (err: any) {
    ...
    <div className="bg-white min-h-screen flex flex-col relative overflow-hidden pb-32">
      ...
      <div className="flex-1 flex flex-col justify-center px-6 py-12 relative z-10">
        ...
        <div className="mb-12">
          ...
          <p className="text-sm font-medium text-slate-600">Sign in to your Service Pilot account</p>
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm font-medium text-slate-600">
            No account?{' '}
            <Link href="/business-setup" className="font-bold text-[#c5724a] hover:text-[#b85f38] transition-colors">
              Setup your business
            </Link>
          </p>
        </div>
