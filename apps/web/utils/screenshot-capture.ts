...
// Only initialize if we're in an actual browser (not during static build)
// Check for interactive/complete readyState to ensure DOM is ready and not in SSR/build context
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  const isInBuildEnvironment = typeof navigator === 'undefined' || !navigator.userAgent;
  if (!isInBuildEnvironment && (document.readyState === 'interactive' || document.readyState === 'complete')) {
    initScreenshotCapture();
  } else if (!isInBuildEnvironment) {
    // Wait for DOM to be ready if loading
    document.addEventListener('DOMContentLoaded', initScreenshotCapture, { once: true });
  }
}
